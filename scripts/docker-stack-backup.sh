...
#######################################
# Docker Compose Stack Backup Script — claudebox
#
# Forked from: https://github.com/TadMSTR/docker-stack-backup
#
# Claudebox deviations from upstream:
#   - STACK_BASE replaces DOCKHAND_BASE + hostname subdirectory logic
#     (claudebox does not use Dockhand; stacks live in ~/docker/)
#   - APPDATA_PATH is /opt/appdata (outside home, avoids Backrest overlap)
#   - BACKUP_DEST is /mnt/atlas/claudebox/docker-backups (NFS to atlas)
#   - Ntfy configured for claudebox topic
#   - No TrueNAS / multi-host layout considerations
#######################################
...
# ── Configuration ─────────────────────────────────────────────────────────────
STACK_BASE="/home/ted/docker"         # Directory containing stack subdirs (each with a compose file)
APPDATA_PATH="/opt/appdata"           # Bind mount root for all stack appdata
BACKUP_DEST="/mnt/atlas/claudebox/docker-backups"
...
EMAIL_ENABLED=false
EMAIL_TO=""
EMAIL_FROM="docker-backup@claudebox"
...
send_critical_restart_failure() {
    ...
    local stack_path="$2"
    local containers="$3"
    local title="CRITICAL: Stack Failed to Restart - claudebox"
...
send_notifications() {
    ...
    if [[ "$status" == "success" ]]; then
        title="Docker Backup Complete - claudebox"
...
Time: $(date +'%Y-%m-%d %H:%M:%S')"
        ...
        title="Docker Backup FAILED - claudebox"
...
backup_stack() {
    ...
    local backup_dir="$BACKUP_DEST/claudebox/$TIMESTAMP"
    mkdir -p "$backup_dir"
